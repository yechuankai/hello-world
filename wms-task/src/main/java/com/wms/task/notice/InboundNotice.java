...
public class InboundNotice extends BaseNotice{
	...
	private static final String url = "/report/html/NOTICEINBOUND?__codelkup=INBOUNDTYPE,INBOUNDSTATUS&id=%s&warehouseId=%s&companyId=%s&userId=0&userName=System&locale=en_US";
	
	@Scheduled(cron = "0 0 8-22 * * ?") //每天8-10点的0分0秒触发
	@RequestMapping("/inbound/notice")
	public void notice() {
		//获取根url
		final String rootUrl = getReportRootUrl();
		//获取配置
		List<CodelkupVO> statusCodelkp = CodelkUpUtils.getCodelkup("INBOUNDSTATUS");
		Map<String, CodelkupVO> statusCodelkpMap = statusCodelkp.stream().collect(Collectors.toMap(CodelkupVO::getCode, v->v));
		//获取标题
		String title = ConfigUtils.getValue(ConfigConstants.CONFIG_NOTIC_SUBJECT_INBOUND);
		if (StringUtils.isEmpty(title))
			return;
		
		IInboundHeaderService inboundHeaderService = SpringUtils.getBean(IInboundHeaderService.class);
		...
		companys.forEach(companyId -> {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				try {
					...
					inbound.setCompanyId(h.getCompanyId());
					inbound.setInboundHeaderId(h.getSourceNumber());
					InboundHeaderTEntity header = inboundHeaderService.findById(inbound);
					...
					//构造报表URL
					String reportUrl = rootUrl + String.format(url, header.getInboundHeaderId(), header.getWarehouseId(), header.getCompanyId() );
