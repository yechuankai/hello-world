(function($){
	...
	//yechuankai
	function cellChange(target, currentInput, cell){
		var dg = $(target);
		var row = dg.datagrid('getRows');
		var oldValue = row[cell.index][cell.field];
		if (oldValue == null){
			oldValue = '';
		}
		...
		var opts = $(target).datagrid('options');
		var tr = opts.finder.getTr(target, cell.index);
		var cellTd = tr.find('td[field="'+cell.field+'"]');
		
		if (oldValue != newValue){ //值没有发生改变，去除选中框
			cellTd.addClass('datagrid-cell-edited');
		}else{
			//cellTd.removeClass('datagrid-cell-edited');
		}
		
		cell['row'] = {};
		tr.find('td.datagrid-cell-edited').each(function(){
			cell['row'][$(this).attr('field')] = true;
		});
		
		return cell;
	}
	//yechuankai
	function highlightCell(target, cell){
		var opts = $(target).datagrid('options');
		var tds = opts.finder.getTr(target, cell.index).find('td');
		tds.each(function(){
			var field = $(this).attr('field');
			if (cell['row'][field]){
				$(this).addClass('datagrid-cell-edited');
			}
		});
			...
			if (input){
				if (accepted){
					if (dg.datagrid('validateRow', cell.index)){
						//yechuankai
						//高亮显示编辑行
						var changes = cellChange(target, input, cell);
						dg.datagrid('endEdit', cell.index);
						highlightCell(target, changes);
	...
	function enableCellEditing(target){
		var dg = $(target);
		var opts = dg.datagrid('options');
		if(opts['enableCellEditingFlag']){return;}
		opts['enableCellEditingFlag'] = true; //标记已经启用
		var panel = dg.datagrid('getPanel');
		panel.attr('tabindex',1).css('outline-style','none').unbind('.cellediting').bind('keydown.cellediting', function(e){
			var h = opts.navHandler[e.keyCode];
			if (h){
	...
	$.extend($.fn.datagrid.methods, {
		...
		input: function(jq, param){
			...
			if (ed){
				var t = $(ed.target);
				if (ed.type == 'combobox' || ed.type == 'combo'){
					t = t.parent().find('.textbox-value');
					return t;
				}
